var board = {
    ...
    doHitDetection: function () {
        ...
        this.teamBNPCs.forEach((Bnpc) => {
            this.teamANPCs.forEach((Anpc) => {
                if (Math.abs(Anpc.pos.x - Bnpc.pos.x) < 0.95 && Math.abs(Anpc.pos.y - Bnpc.pos.y) < 0.95) {
                    Anpc.hitDetected();
                    Bnpc.hitDetected();
                }
            });
            ...
            this.teamBNPCs.forEach((teammate) => {
                if (Math.abs(teammate.pos.x - Bnpc.pos.x) < 0.95 && Math.abs(teammate.pos.y - Bnpc.pos.y) < 0.95) {
            ...
            if (Math.abs(this.Avtr.pos.x - Bnpc.pos.x) < 0.95 && Math.abs(this.Avtr.pos.y - Bnpc.pos.y) < 0.95) {
        ...
        this.teamANPCs.forEach((Anpc) => {
            this.teamANPCs.forEach((teammate) => {
                if (Math.abs(teammate.pos.x - Anpc.pos.x) < 0.95 && Math.abs(teammate.pos.y - Anpc.pos.y) < 0.95) {
                    teammate.hitDetected();
                    Anpc.hitDetected();
                }
            });

            if (Math.abs(this.Avtr.pos.x - Anpc.pos.x) < 0.95 && Math.abs(this.Avtr.pos.y - Anpc.pos.y) < 0.95) {
                this.Avtr.hitDetected();
                Anpc.hitDetected();
            }
        });
